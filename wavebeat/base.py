...
class Base(pl.LightningModule):
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        #     "perf": make_table(songs),
        #     "global_step": self.global_step
        #     })
        #self.logger.log_text(key="perf", columns=[self.global_step], data=make_table(songs))
        #self.logger.experiment.log(key="perf", columns=[self.global_step], data=make_table(songs))
        '''
        self.logger.experiment.log({
            f"hist/F-measure": wandb.Image(plot_histogram(songs)),
            "global_step": self.global_step,
            "perf": wandb.Table(columns=[self.global_step], data=[[make_table(songs)]])
            })
        ...
        for idx, rand_idx in enumerate(list(rand_indices)):
            ...
            #                                                   song_name=f),
            #                                  self.global_step)
            img_1 =  plot_activations(ref_beats, 
                                        est_beats, 
                                        est_sm,
                                        self.hparams.target_sample_rate,
                                        ref_downbeats=ref_downbeats,
            ...
            columns = [f"input/{idx}", f"act/{idx}"]
            data = [[wandb.Image(img_1), wandb.Audio(i, sample_rate=self.hparams.audio_sample_rate)]]
            self.logger.experiment.log({
                f"samples: {self.global_step}": wandb.Table(columns=columns, data=data)
                })
            # self.logger.experiment.log({
            #     f"samples: {self.global_step}", wandb.Image(img_1),
